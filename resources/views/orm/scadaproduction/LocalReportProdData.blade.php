...
@foreach($ProdData as $ProdDataLocal)
	<tr>
		...
		<td
			@php 
				$rem_style = empty($ProdDataLocal->remarks1) ? 'btn btn-sm btn-link btn-danger' : '';
		...
		<td>{{ $ProdDataLocal->VAST_status }}</td>{{-- CSP --}}

		<td>{{ $ProdDataLocal->LL_status }}</td>
		...
		<td
			@php 
				$rem_style = empty($ProdDataLocal->remarks2) ? 'btn btn-sm btn-link btn-danger' : '';
			@endphp
		>
